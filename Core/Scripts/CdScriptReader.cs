...
{
    public class CdScriptReader : IScriptReader
    {
        public string Tag => "CD/Xuse";
        public string Description => "Xuse CD script file";
        ...
        {
            0x3B, 0x40, 0x41, 0x44, 0x61, 0x62, 0x63, 0x64, 0x65
        };
        ...
        private static List<Instruction> ParseInstructions(byte[] secondData)
        {
            ...
            while (pos + 12 <= secondData.Length)
            {
                ...
                if (inst.HasText)
                {
                    ...
                    {
                        inst.A0 = BitConverter.ToUInt16(secondData, pos + 2);
                        inst.ThirdOffset = (int)BitConverter.ToUInt32(secondData, pos + 4);
                    }
                    else if (cmd == 0x3B)
                    {
                        inst.StrLen = BitConverter.ToUInt16(secondData, pos + 2);
                        inst.ThirdOffset = (int)BitConverter.ToUInt32(secondData, pos + 4);
                    }
                    else if (cmd == 0x44)
                    {
                    ...
                    else if (cmd >= 0x61 && cmd <= 0x65)
                    {
                        inst.Cnt = BitConverter.ToUInt16(secondData, pos + 2);
                        inst.ThirdOffset = (int)BitConverter.ToUInt32(secondData, pos + 4);
                    }
                }
        ...
        private static List<string> ExtractShowText(Instruction inst, byte[] thirdData)
        {
            ...
            uint a = BitConverter.ToUInt32(thirdData, pos); pos += 4;
            uint off2 = BitConverter.ToUInt32(thirdData, pos); pos += 4;
            uint idx = BitConverter.ToUInt32(thirdData, pos); pos += 4;
        ...
        private byte[] ImportText(byte[] originalData, List<ScriptEntry> modifiedEntries)
        {
            ...
            foreach (var entry in modifiedEntries)
            {
                if (!string.IsNullOrEmpty(entry.Text))
                    translations[entry.Index] = entry.Text.Split('\n');
            }

            if (translations.Count == 0)
                return originalData;

            var thirdRefs = new List<(int idx, ushort cmd, int offset, Instruction inst)>();
            for (int idx = 0; idx < instructions.Count; idx++)
            {
                ...
                thirdRefs.Add((idx, inst.Cmd, off, inst));
            }
            thirdRefs.Sort((a, b) => a.offset.CompareTo(b.offset));
            ...
            foreach (var (idx, cmd, trdPos, inst) in thirdRefs)
            {
                ...
                {
                    int regionSize = GetThirdRegionSize(cmd, thirdData, trdPos, secondData, inst.Pos);
                    newThirdMs.Write(thirdData, trdPos, regionSize);
                }
            }

            foreach (var (idx, _, _, inst) in thirdRefs)
            {
                if (offsetRemap.TryGetValue(idx, out int newOff))
            ...
            if (tailStart < raw.Length)
                outMs.Write(raw, tailStart, raw.Length - tailStart);

            return outMs.ToArray();
        }

        private void RebuildThirdEntry(ushort cmd, Instruction inst, int trdPos, byte[] thirdData,
            byte[] secondData, string[] newStrings, MemoryStream newThird, List<Instruction> instructions)
        {
            switch (cmd)
            {
                case 0x40:
                case 0x41:
                {
                    ...
                    var encStrs = new List<byte[]>();
                    foreach (var s in newStrings)
                        encStrs.Add(EncodeText(s));
                    ...
                    newThird.Write(bufBytes, 0, bufBytes.Length);
                    break;
                }
                case 0x3B:
                {
                    byte[] newStr = ShiftJis.GetBytes(newStrings[0]);
                    newThird.Write(newStr, 0, newStr.Length);
                    BitConverter.GetBytes((ushort)newStr.Length).CopyTo(secondData, inst.Pos + 2);
                    break;
                }
                case 0x44:
                {
                ...
                case 0x62:
                {
                    ...
                    foreach (var s in encStrs)
                    {
                        newThird.Write(BitConverter.GetBytes((uint)s.Length), 0, 4);
                        newThird.Write(s, 0, s.Length);
                    }
                    BitConverter.GetBytes((ushort)newStrings.Length).CopyTo(secondData, inst.Pos + 2);
                    break;
                }
                case 0x63:
                {
                    ...
                    newThird.Write(BitConverter.GetBytes(aVal), 0, 4);
                    foreach (var s in encStrs)
                    {
                        newThird.Write(BitConverter.GetBytes((uint)s.Length), 0, 4);
                        newThird.Write(s, 0, s.Length);
                ...
                {
                    int cnt2 = inst.Cnt;
                    newThird.Write(thirdData, trdPos, cnt2 * 4);
                    var encStrs = new List<byte[]>();
                    foreach (var s in newStrings)
                        encStrs.Add(EncodeText(s));
                    foreach (var s in encStrs)
                ...
                case 0x65:
                {
                    ...
                    int cnt2 = inst.Cnt;
                    newThird.Write(BitConverter.GetBytes(aVal2), 0, 4);
                    newThird.Write(thirdData, trdPos + 4, cnt2 * 4);
                    var encStrs = new List<byte[]>();
                    foreach (var s in newStrings)
                        encStrs.Add(EncodeText(s));
                    foreach (var s in encStrs)
                ...
                {
                    int regionSize = GetThirdRegionSize(cmd, thirdData, trdPos, secondData, inst.Pos);
                    newThird.Write(thirdData, trdPos, regionSize);
                    break;
                }
            }
        }
        ...
        {
            return (int)BitConverter.ToUInt32(secondData, instPos + 4);
        }
        ...
        private static int GetThirdRegionSize(ushort cmd, byte[] thirdData, int trdPos, byte[] secondData, int instPos)
        {
            switch (cmd)
            {
                ...
                case 0x76: return 12;
                case 0x77: case 0x78: return 20;
                case 0x79: return 37;
                case 0x7A: return 22;
                case 0x7B: return 5;
                case 0x7C: case 0x85: return 6;
                case 0x7D: return 7;
                case 0x7E: case 0x7F: case 0x80: return 13;
        ...
        private static int Sum(List<byte[]> list, Func<byte[], int> selector)
        {
            int total = 0;
